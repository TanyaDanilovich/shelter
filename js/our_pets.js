...
let navigation_aActive = document.querySelector('.nav-pets-active');

HAMBURGER.addEventListener('click', () => toggleClasses())

NAVIGATION_ITEM.forEach((li) => {
	li.addEventListener('click', (() => toggleClasses()))
})
...
const toggleClasses = () => {
	...
	NAVIGATION.classList.toggle('hamburger-pets-navigation')
...
document.querySelector('#pets-list').addEventListener('click', (event) => {
	let petEvent;
	...
	if (event.target.classList.contains('card-img')) {
		...
		//console.log('active-imgcard-img', event.target.parentElement);
	}
